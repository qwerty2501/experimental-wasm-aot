...
mod tests{
    ...
    #[test]
    pub fn sort_test_works()->Result<(),Error>{
        let return_only_module =  load_wasm_compiler_test_case("sort_test")?;
        let wasm_compiler = WasmCompiler::<u32>::new();
        let context = Context::new();
        let module_id = "sort_test";
        let build_context = BuildContext::new(module_id,&context);
        let function_name = WasmCompiler::<u32>::wasm_function_name("sort_test");
        wasm_compiler.build_main_function(&build_context,module_id,&return_only_module,WasmCompiler::<u32>::set_declare_main_function(&build_context),||{
            let target_function = build_context.module().get_named_function(&function_name).ok_or(NoSuchLLVMFunction {name:function_name})?;
            build_context.builder().build_ret( build_context.builder().build_call(target_function,&[],""));
            Ok(())
    ...
    #[test]
    pub fn lt_u_test_works()->Result<(),Error>{
        let return_only_module =  load_wasm_compiler_test_case("lt_u_test")?;
        let wasm_compiler = WasmCompiler::<u32>::new();
        let context = Context::new();
        let module_id = "lt_u_test";
        let build_context = BuildContext::new(module_id,&context);
        let function_name = WasmCompiler::<u32>::wasm_function_name("lt_u_test");
        wasm_compiler.build_main_function(&build_context,module_id,&return_only_module,WasmCompiler::<u32>::set_declare_main_function(&build_context),||{
            let target_function = build_context.module().get_named_function(&function_name).ok_or(NoSuchLLVMFunction {name:function_name})?;
            build_context.builder().build_ret( build_context.builder().build_call(target_function,&[],""));
            Ok(())
    ...
    #[test]
    pub fn gt_u_test_works()->Result<(),Error>{
        let case_name = "gt_u_test";
    ...
    pub fn pow_works()->Result<(),Error>{
        let return_only_module =  load_wasm_compiler_test_case("pow")?;
        let wasm_compiler = WasmCompiler::<u32>::new();
        let context = Context::new();
        let module_id = "pow";
        let build_context = BuildContext::new(module_id,&context);
        let function_name = WasmCompiler::<u32>::wasm_function_name("pow");
        wasm_compiler.build_main_function(&build_context,module_id,&return_only_module,WasmCompiler::<u32>::set_declare_main_function(&build_context),||{
            let target_function = build_context.module().get_named_function(&function_name).ok_or(NoSuchLLVMFunction {name:function_name})?;
